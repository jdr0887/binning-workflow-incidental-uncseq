...
import org.renci.binning.incidental.uncseq.commons.LoadMissingCallable;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class LoadMissingDelegate implements JavaDelegate {

    private static final Logger logger = LoggerFactory.getLogger(LoadMissingDelegate.class);

    public LoadMissingDelegate() {
        super();
    }
    ...
    public void execute(DelegateExecution execution) throws Exception {
        ...
        try {
            ...
            binningJob.setStatus(daoBean.getIncidentalStatusTypeDAO().findById("Loading Missing"));
            daoBean.getIncidentalBinningJobDAO().save(binningJob);
            logger.info(binningJob.toString());

            ExecutorService es = Executors.newSingleThreadExecutor();
            Future<Void> future = es.submit(new LoadMissingCallable(execution.getVariables()));
            es.shutdown();
            es.awaitTermination(1L, TimeUnit.HOURS);

            binningJob = daoBean.getIncidentalBinningJobDAO().findById(binningJobId);
            binningJob.setStatus(daoBean.getIncidentalStatusTypeDAO().findById("Loaded Missing"));
            daoBean.getIncidentalBinningJobDAO().save(binningJob);
            logger.info(binningJob.toString());
